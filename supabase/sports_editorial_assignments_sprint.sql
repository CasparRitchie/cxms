...
create or replace function public.sports_editorial_provision_user(
  p_workspace_id uuid, p_email text, p_full_name text, p_password_hash text, p_editorial_role text
)
returns table (user_id uuid, existing_account boolean)
language plpgsql security definer set search_path = public
as $$
declare v_user_id uuid; v_existing boolean := false;
begin
  if p_editorial_role not in ('researcher', 'sub_editor', 'supervisor') then raise exception 'Invalid Sports Editorial role'; end if;
  ...
  if v_user_id is null then
    ...
    values (lower(trim(p_email)), p_password_hash, nullif(trim(p_full_name), ''), true) returning id into v_user_id;
  else v_existing := true; end if;
  if not exists (select 1 from public.workspace_members where workspace_id=p_workspace_id and user_id=v_user_id) then
    insert into public.workspace_members (workspace_id,user_id,email,role) values (p_workspace_id,v_user_id,lower(trim(p_email)),'member');
  end if;
  ...
  on conflict on constraint sports_editorial_memberships_workspace_id_user_id_key do update
    set editorial_role=excluded.editorial_role,is_active=true;
