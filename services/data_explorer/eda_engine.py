...
def parse_csv_dataset(raw_data, max_rows=5000, max_columns=50):
    """
    Parse pasted CSV text into a pandas DataFrame.

    This function intentionally raises ValueError for user-facing validation errors
...
def build_eda_report(df):
    ...
    We keep the original dataframe for quality reporting, then create an
    analysis dataframe with safe derived features such as has_Cabin.
    """
    original_df = df.copy()

    # First pass on original data
    quality = build_quality_report(original_df)

    # Add derived features for analysis without overwriting original data
    ...
    charts = recommend_charts(analysis_df, schema)
...
def infer_schema(df):
    ...
    - Missing values should not stop a column being numeric/date if the non-missing values fit.
    ...
    - Low-cardinality numeric columns can still be used later as categorical-like fields.
    """
    schema = {
        "numeric": [],
        "categorical": [],
    ...
    for column in df.columns:
        ...
        # Datetime detection before ID/numeric detection.
...
def add_derived_features(df, quality, missing_threshold=30):
    ...
    For high-missingness columns, create a boolean indicator:
    has_<column> = True when the original value is present.

    Example:
    Cabin missing in 77% of Titanic rows
    -> has_Cabin tells us whether cabin information exists.
    """
    analysis_df = df.copy()
    derived_features = []
        ...
        derived_features.append(
            {
                ...
                "reason": (
                    ...
                    "Missingness may be analytically meaningful, so a boolean "
                    "presence indicator was created."
                ),
            }
        )
...
def make_derived_feature_name(base_name, existing_columns):
    """
    Create a safe derived feature name without colliding with existing columns.
    """
...
def build_quality_report(df):
    ...
    return {
        ...
        "missing_percent_total": round(
            (total_missing_cells / max(total_cells, 1)) * 100,
            2,
        ),
...
def get_categorical_like_columns(df, schema, max_unique=12):
    """
    Return true categorical columns plus low-cardinality numeric columns
    that behave like categories, e.g. Titanic Pclass or Survived.
    """
...
def recommend_charts(df, schema):
    charts = []
    ...
    # Advanced / target-aware charts
    charts.extend(build_target_charts(df, schema))
    charts.extend(build_segmentation_charts(df, schema))
...
def build_numeric_charts(df, numeric_cols):
    ...
    for column in numeric_cols:
        ...
        charts.append(
            {
                "id": safe_chart_id(f"histogram_{column}"),
                "title": f"Distribution of {column}",
                "chart_type": "histogram",
                "data_role": "numeric",
                "phase": "univariate",
                "columns": [column],
                "values": numeric.head(500).tolist(),
                "why": "Shows the distribution, spread and possible skew of a numeric field.",
            }
        )

        charts.append(
            {
                "id": safe_chart_id(f"boxplot_{column}"),
                ...
                "columns": [column],
                "values": numeric.head(500).tolist(),
                "why": "Highlights median, quartiles and possible outliers.",
            }
        )
...
def build_categorical_charts(df, categorical_cols):
    ...
    for column in categorical_cols[:10]:
        ...
        charts.append(
            {
                "id": safe_chart_id(f"bar_{column}"),
                ...
                "why": "Shows the most common groups or categories.",
            }
        )
    ...
    return [
        {
            ...
            "data_role": "relational",
            "phase": "bivariate",
            "columns": numeric_cols,
            "matrix": corr_df.round(3).replace({np.nan: None}).to_dict(),
            "why": "Shows which numeric fields move together or against each other.",
        }
    ]
...
def build_categorical_numeric_charts(df, categorical_cols, numeric_cols):
    ...
    for cat in categorical_cols[:5]:
        for num in numeric_cols[:5]:
            ...
            charts.append(
                {
                    "id": safe_chart_id(f"mean_{num}_by_{cat}"),
                    "title": f"Average {num} by {cat}",
                    "chart_type": "grouped_bar",
                    "data_role": "categoric_numeric",
                    "phase": "bivariate",
                    "columns": [cat, num],
                    "labels": [str(index) for index in grouped.index],
                    "values": [safe_round(value) for value in grouped.values],
                    "why": "Compares a numeric measure across categories.",
                }
            )
...
def build_time_series_charts(df, datetime_cols, numeric_cols):
    ...
    for date_col in datetime_cols[:3]:
        ...
        for num in numeric_cols[:5]:
            ...
            charts.append(
                {
                    "id": safe_chart_id(f"time_{num}_by_{date_col}"),
                    "title": f"{num} over time by {date_col}",
                    "chart_type": "line",
                    "data_role": "time_series",
                    "phase": "time_series",
                    "columns": [date_col, num],
                    "labels": [str(index.date()) for index in monthly.index],
                    "values": [safe_round(value) for value in monthly.values],
                    "why": "Shows movement, trend or seasonality over time.",
                }
            )
...
def build_scatter_charts(df, numeric_cols):
    """
    Build scatter plots for pairs of numeric columns.
    Useful for relationship and outlier analysis.
    """
    charts = []
    ...
    for i, x_col in enumerate(numeric_cols):
        for y_col in numeric_cols[i + 1:]:
            ...
            charts.append(
                {
                    "id": safe_chart_id(f"scatter_{x_col}_vs_{y_col}"),
                    "title": f"{y_col} vs {x_col}",
                    "chart_type": "scatter",
                    "data_role": "numeric_numeric",
                    "phase": "bivariate",
                    "columns": [x_col, y_col],
                    "x": temp[x_col].tolist(),
                    "y": temp[y_col].tolist(),
                    "x_label": x_col,
                    "y_label": y_col,
                    "why": "Shows the relationship, clustering and possible outliers between two numeric fields.",
                }
            )
...
def build_bubble_charts(df, numeric_cols):
    """
    Build bubble charts when at least three numeric columns exist.
    """
    charts = []
    ...
    # Avoid invisible bubbles if values are tiny or all identical.
    if size_values.max() == size_values.min():
        marker_sizes = [16 for _ in size_values]
    else:
        marker_sizes = (
            8 + 32 * ((size_values - size_values.min()) / (size_values.max() - size_values.min()))
        ).tolist()

    charts.append(
        {
            "id": safe_chart_id(f"bubble_{x_col}_{y_col}_size_{size_col}"),
            "title": f"{y_col} vs {x_col}, sized by {size_col}",
            "chart_type": "bubble",
            "data_role": "numeric_numeric_numeric",
            "phase": "multivariate",
            "columns": [x_col, y_col, size_col],
            "x": temp[x_col].tolist(),
            "y": temp[y_col].tolist(),
            "size": marker_sizes,
            "x_label": x_col,
            "y_label": y_col,
            "size_label": size_col,
            "why": "Adds a third numeric measure as bubble size to show a richer multivariate relationship.",
        }
    )
...
def build_category_distribution_charts(df, categorical_cols, numeric_cols):
    """
    Build boxplot and violin charts for numeric columns split by category.
    """
    charts = []
    ...
    for cat_col in categorical_cols[:5]:
        ...
        # Avoid unreadable charts with too many groups.
        ...
        for num_col in numeric_cols[:5]:
            ...
            charts.append(
                {
                    "id": safe_chart_id(f"boxplot_{num_col}_by_{cat_col}"),
                    "title": f"{num_col} distribution by {cat_col}",
                    "chart_type": "boxplot_by_category",
                    "data_role": "categoric_numeric",
                    "phase": "bivariate",
                    "columns": [cat_col, num_col],
                    "x": temp[cat_col].astype(str).tolist(),
                    "y": temp[num_col].tolist(),
                    "x_label": cat_col,
                    "y_label": num_col,
                    "why": "Compares medians, spread and outliers across categories.",
                }
            )

            charts.append(
                {
                    "id": safe_chart_id(f"violin_{num_col}_by_{cat_col}"),
                    "title": f"{num_col} shape by {cat_col}",
                    "chart_type": "violin_by_category",
                    "data_role": "categoric_numeric",
                    "phase": "bivariate",
                    "columns": [cat_col, num_col],
                    "x": temp[cat_col].astype(str).tolist(),
                    ...
                    "x_label": cat_col,
                    "y_label": num_col,
                    "why": "Shows the distribution shape of a numeric field within each category.",
                }
            )
...
def build_stacked_bar_charts(df, categorical_cols):
    """
    Build stacked bar charts for pairs of categorical columns.
    """
    charts = []
    ...
    for i, x_col in enumerate(categorical_cols):
        for stack_col in categorical_cols[i + 1:]:
            ...
            if x_unique < 2 or stack_unique < 2:
                continue

            if x_unique > 12 or stack_unique > 8:
                continue
            ...
            charts.append(
                {
                    "id": safe_chart_id(f"stacked_bar_{x_col}_by_{stack_col}"),
                    "title": f"{x_col} split by {stack_col}",
                    "chart_type": "stacked_bar",
                    "data_role": "categoric_categoric",
                    "phase": "bivariate",
                    "columns": [x_col, stack_col],
                    "x_labels": [str(index) for index in counts.index],
                    "series": [
                        {
                            "name": str(column),
                            "values": [int(value) for value in counts[column].values],
                        }
                        for column in counts.columns
                    ],
                    "x_label": x_col,
                    "stack_label": stack_col,
                    "why": "Shows the composition of one categorical field within another.",
                }
            )
...
def build_area_charts(df, datetime_cols, numeric_cols):
    """
    Build area charts for numeric values over time.
    """
    charts = []
    ...
    for date_col in datetime_cols[:2]:
        ...
        for num_col in numeric_cols[:4]:
            ...
            charts.append(
                {
                    "id": safe_chart_id(f"area_{num_col}_by_{date_col}"),
                    "title": f"{num_col} area trend over time",
                    "chart_type": "area",
                    "data_role": "time_series",
                    "phase": "time_series",
                    ...
                    "labels": [str(index.date()) for index in monthly.index],
                    "values": [safe_round(value) for value in monthly.values],
                    "why": "Shows the size and movement of a numeric measure over time.",
                }
            )
...
def build_target_charts(df, schema):
    ...
    charts.append(
        {
            "id": safe_chart_id(f"target_distribution_{target_column}"),
            "title": f"Target distribution: {target_column}",
            "chart_type": "bar",
            "data_role": "target",
            "phase": "advanced",
            "columns": [target_column],
            "labels": [row["target_value"] for row in distribution],
            "values": [row["count"] for row in distribution],
            "why": "Shows how the likely target or outcome column is distributed.",
        }
    )
    ...
    categorical_cols = [
        col for col in get_categorical_like_columns(df, schema)
        if col != target_column
    ]

    # Binary/numeric target rate by category.
    if target_is_numeric:
        for column in categorical_cols[:6]:
            ...
            grouped = (
                temp.groupby(column)[target_column]
                .mean()
                .sort_values(ascending=False)
                .head(15)
            )
            ...
            charts.append(
                {
                    "id": safe_chart_id(f"target_rate_{target_column}_by_{column}"),
                    "title": f"Average {target_column} by {column}",
                    "chart_type": "bar",
                    "data_role": "target",
                    "phase": "advanced",
                    ...
                    "labels": [str(index) for index in grouped.index],
                    "values": [safe_round(value) for value in grouped.values],
                    "why": "Shows how the target or outcome varies by category.",
                }
            )
...
def build_segmentation_charts(df, schema):
    ...
    charts = [
        {
            "id": "pca_scatter_by_cluster",
            "title": "PCA projection coloured by KMeans cluster",
            "chart_type": "pca_scatter",
            "data_role": "segmentation",
            "phase": "advanced",
            "columns": segmentation["numeric_columns"],
            "x": [point["pc1"] for point in points],
            "y": [point["pc2"] for point in points],
            "colour": [str(point["cluster"]) for point in points],
            "hover": [
                f"Row {point['row_index']} | Cluster {point['cluster']}"
                for point in points
            ],
            "x_label": f"PC1 ({segmentation['pca']['explained_variance_pc1']}%)",
            "y_label": f"PC2 ({segmentation['pca']['explained_variance_pc2']}%)",
            "why": "Uses PCA to reduce numeric features into two dimensions, then colours rows by KMeans cluster.",
        }
    ]
    ...
    if has_target:
        charts.append(
            {
                "id": "pca_scatter_by_target",
                "title": "PCA projection coloured by detected target",
                "chart_type": "pca_scatter",
                "data_role": "segmentation",
                "phase": "advanced",
                "columns": segmentation["numeric_columns"],
                "x": [point["pc1"] for point in points],
                "y": [point["pc2"] for point in points],
                "colour": [str(point["target"]) for point in points],
                "hover": [
                    f"Row {point['row_index']} | Target {point['target']}"
                    for point in points
                ],
                "x_label": f"PC1 ({segmentation['pca']['explained_variance_pc1']}%)",
                "y_label": f"PC2 ({segmentation['pca']['explained_variance_pc2']}%)",
...
def analyse_relationships(df, schema):
    ...
    return {
        ...
        "message": "Correlation is not causation, but these relationships may deserve investigation.",
    }
...
def analyse_nearest_neighbours(df, schema):
    ...
    if len(numeric_cols) < 2 or len(df) < 5:
        return {
            "available": False,
            "message": "Not enough numeric data for nearest-neighbour analysis.",
        }
    ...
    if len(numeric_df) < 5:
        return {
            "available": False,
            "message": "Not enough complete numeric rows for nearest-neighbour analysis.",
        }
    ...
    return {
        ...
        "reference_row_index": int(sample.index[0]),
        "nearest_rows": [
            {
                "row_index": int(sample.index[idx]),
                "distance": safe_round(dist),
            }
            for idx, dist in zip(indices[0], distances[0])
        ],
        "message": "Nearest-neighbour analysis finds rows with similar numeric profiles. This can later power segmentation and lookalike analysis.",
    }
...
def generate_theses(df, schema, quality, relationships):
    ...
    if quality["missing_percent_total"] > 10:
        theses.append(
            {
                "title": "Data quality may be limiting confidence in the analysis",
                "evidence": f"{quality['missing_percent_total']}% of all cells are missing.",
                "input_level_improvement": "Review the data collection process and make key fields mandatory where possible.",
                "operational_improvement": "Avoid making major decisions from columns with high missingness until the capture process is improved.",
            }
        )
    ...
    for relationship in relationships.get("strong_correlations", [])[:5]:
        theses.append(
            {
                "title": f"{relationship['column_a']} and {relationship['column_b']} appear related",
                "evidence": (
                    f"The correlation is {relationship['correlation']}, "
                    f"which is {relationship['strength']} and {relationship['direction']}."
                ),
                "input_level_improvement": "Check whether both fields are captured consistently and whether either is derived from the other.",
                "operational_improvement": "Investigate whether movement in one field can help explain or predict movement in the other.",
            }
        )
    ...
    if schema["datetime"] and schema["numeric"]:
        theses.append(
            {
                "title": "Time-based trends may explain changes in performance",
                "evidence": (
                    f"Detected date fields: {', '.join(schema['datetime'])}. "
                    "Numeric fields can be trended over time."
                ),
                "input_level_improvement": "Ensure dates are captured in a consistent format.",
                "operational_improvement": "Use monthly or weekly trend views to separate one-off variation from genuine change.",
            }
        )
    ...
    if schema["categorical"] and schema["numeric"]:
        theses.append(
            {
                "title": "Performance may vary by group or segment",
                "evidence": (
                    f"Detected categorical fields such as {', '.join(schema['categorical'][:3])} "
                    f"and numeric fields such as {', '.join(schema['numeric'][:3])}."
                ),
                "input_level_improvement": "Make sure category labels are standardised to avoid duplicate groups caused by spelling or formatting differences.",
                "operational_improvement": "Compare averages by segment to identify which groups may need targeted improvement.",
            }
        )
    ...
    if not theses:
        theses.append(
            {
                "title": "The dataset is valid but may need richer fields for deeper insight",
                "evidence": "The tool did not find strong missingness, time trends or numeric relationships in the first pass.",
                "input_level_improvement": "Add dates, categories, outcomes and operational context fields to make future analysis more useful.",
                "operational_improvement": "Use this dataset as a baseline and compare it with future extracts.",
            }
        )
...
def build_segmentation_report(df, schema):
    """
    Build PCA + KMeans segmentation analysis for numeric datasets.
    ...
    if len(numeric_cols) < 2:
        return {
            "available": False,
            "message": "At least two numeric columns are needed for PCA and clustering.",
    ...
    if len(numeric_df) < 10:
        return {
            "available": False,
            "message": "At least ten complete numeric rows are needed for useful PCA and clustering.",
            "numeric_columns": numeric_cols,
            "selected_k": None,
            "k_scores": [],
            "pca": None,
    ...
    for position, index in enumerate(sample.index):
        points.append(
            {
                "row_index": int(index),
                "pc1": safe_round(pca_points[position, 0]),
                "pc2": safe_round(pca_points[position, 1]),
                "cluster": int(cluster_labels[position]),
                "target": target_values[position] if target_values else None,
            }
        )
    ...
    return {
        "available": True,
        "message": (
            f"Built a 2D PCA projection and KMeans segmentation using "
            f"{len(sample)} complete rows and {len(numeric_cols)} numeric columns. "
            f"Selected K={selected_k} using silhouette score."
        ),
        "numeric_columns": numeric_cols,
        "selected_k": selected_k,
        "cluster_count": selected_k,
        "k_scores": k_selection["k_scores"],
        "pca": {
            "explained_variance_pc1": safe_round(pca.explained_variance_ratio_[0] * 100, 2),
            "explained_variance_pc2": safe_round(pca.explained_variance_ratio_[1] * 100, 2),
            "total_explained_variance": safe_round(pca.explained_variance_ratio_.sum() * 100, 2),
            "loadings": pca_loadings,
            "points": points,
        },
        "clusters": {
            "labels": [int(label) for label in cluster_labels],
            "inertia": safe_round(kmeans.inertia_),
        },
...
def choose_best_kmeans_k(scaled_data):
    """
    Try KMeans for k=2..8 and select the k with the best silhouette score.

    Falls back safely when the dataset is too small.
    """
    ...
    for k in range(2, max_k + 1):
        try:
            ...
            result = {
                "k": int(k),
                "inertia": safe_round(model.inertia_),
                "silhouette": safe_round(score),
            }
...
def build_pca_loadings(pca, numeric_cols):
    """
    PCA loadings show which original numeric columns drive PC1 and PC2.
    """
    loadings = []
    ...
    for component_index, component_name in enumerate(["PC1", "PC2"]):
        ...
        for column, loading in zip(numeric_cols, component):
            rows.append(
                {
                    "column": column,
                    "loading": safe_round(loading),
                    "absolute_loading": safe_round(abs(loading)),
...
def get_target_values_for_indices(df, indices, target_column):
    ...
    return [
        ...
        for value in df.loc[indices, target_column].tolist()
    ]
...
def build_cluster_profiles(sample, numeric_cols, cluster_labels):
    ...
    for cluster_id, cluster_df in grouped:
        ...
        for column in numeric_cols:
            profile["numeric_summary"].append(
                {
                    "column": column,
                    "mean": safe_round(cluster_df[column].mean()),
                    "median": safe_round(cluster_df[column].median()),
                    "min": safe_round(cluster_df[column].min()),
                    "max": safe_round(cluster_df[column].max()),
                }
            )
...
def build_advanced_report(df, schema):
    """
    Build higher-level EDA insights:
    - likely target detection
    - target distribution
    - target by category
    - numeric averages by target
    """
    target_column = detect_target_column(df, schema)
    ...
    if not target_column:
        return {
            ...
            "message": "No obvious target/outcome column detected yet.",
            "target": None,
            "target_distribution": [],
            "target_by_category": [],
            "numeric_by_target": [],
    ...
    return {
        ...
        "message": f"Detected '{target_column}' as a likely target or outcome column.",
        "target": target_column,
        "target_distribution": build_target_distribution(df, target_column),
        "target_by_category": build_target_by_category(df, schema, target_column),
        "numeric_by_target": build_numeric_by_target(df, schema, target_column),
    }


def prepare_ml_features(df, schema, target_column):
    """
    Prepare features for supervised ML.

    Includes:
    - numeric columns
    - boolean columns converted to 0/1
    - low-cardinality categorical columns one-hot encoded
    - excludes likely IDs, text and high-cardinality fields
    - median-imputes numeric fields
    ...
    # Numeric features: convert and median-impute.
    for column in numeric_cols:
    ...
    # Categorical features: fill missing and one-hot encode.
    if categorical_cols:
...
def build_feature_importance(feature_columns, importances, limit=15):
    """
    Convert model feature importances into a frontend-friendly list.
    """
    rows = []
    ...
    for column, importance in zip(feature_columns, importances):
        rows.append(
            {
                "feature": column,
                "importance": safe_round(importance, 5),
            }
        )
...
def build_ml_report(df, schema):
    """
    First supervised ML pass using KNN.

    Detects a target column, decides classification vs regression,
    prepares numeric, boolean and one-hot encoded categorical features,
    and evaluates a KNN model.
    """
    target_column = detect_target_column(df, schema)
    ...
    if not target_column:
        return {
            "available": False,
            "message": "No target column detected, so supervised ML was not run.",
            "target": None,
            "task_type": None,
            "feature_columns": [],
            "feature_groups": {},
    ...
    if X is None or len(feature_columns) < 2:
        return {
            "available": False,
            "message": "At least two usable feature columns are needed for this first ML pass.",
            "target": target_column,
            "task_type": None,
    ...
    if len(model_df) < 30:
        return {
            "available": False,
            "message": "At least 30 complete target rows are recommended before running supervised ML.",
            "target": target_column,
            "task_type": None,
            "feature_columns": feature_columns,
            "feature_groups": feature_groups,
    ...
    if task_type == "classification":
        return build_knn_classification_report(
            model_df=model_df,
            feature_columns=feature_columns,
    ...
    if task_type == "regression":
        return build_knn_regression_report(
            model_df=model_df,
            feature_columns=feature_columns,
            target_column=target_column,
            feature_groups=feature_groups,
    ...
    return {
        "available": False,
        "message": "Could not confidently infer whether the target is classification or regression.",
        "target": target_column,
        "task_type": None,
        "feature_columns": feature_columns,
        "feature_groups": feature_groups,
...
def build_knn_classification_report(model_df, feature_columns, target_column, feature_groups=None):
    ...
    if len(set(y)) < 2:
        return {
            "available": False,
            "message": "The detected target has only one class after cleaning, so classification cannot run.",
            "target": target_column,
    ...
    # KNN
    neighbour_count = min(5, len(X_train))
    ...
    models.append(
        {
            ...
            "model_key": "knn_classifier",
            "score_name": "accuracy",
            "score": safe_round(knn_accuracy, 4),
            "neighbours": int(neighbour_count),
            "notes": "Predicts the target from nearby rows in scaled numeric, boolean and encoded categorical feature space.",
        }
    )
    ...
    models.append(
        {
            ...
            "score_name": "accuracy",
            "score": safe_round(rf_accuracy, 4),
            "notes": "Builds many decision trees and combines them. Useful for non-linear patterns and feature importance.",
    ...
    if best_model["model_key"] == "random_forest_classifier":
    ...
    return {
        "available": True,
        "message": (
            f"Ran first-pass classification models to predict '{target_column}'. "
            "These are exploratory and should not be treated as production models."
        ),
        "target": target_column,
        "task_type": "classification",
        "feature_columns": feature_columns,
...
def build_knn_regression_report(model_df, feature_columns, target_column, feature_groups=None):
    ...
    if len(model_df) < 30:
        return {
            "available": False,
            "message": "At least 30 complete rows are recommended before running regression.",
            "target": target_column,
    ...
    models.append(
        {
            ...
            "model_key": "knn_regressor",
            "score_name": "R²",
            "score": safe_round(knn_r2, 4),
            "mae": safe_round(knn_mae, 4),
            "rmse": safe_round(knn_rmse, 4),
            "neighbours": int(neighbour_count),
            "notes": "Predicts the numeric target from nearby rows in scaled numeric, boolean and encoded categorical feature space.",
        }
    )
    ...
    models.append(
        {
            ...
            "score_name": "R²",
            "score": safe_round(rf_r2, 4),
            "mae": safe_round(rf_mae, 4),
            "rmse": safe_round(rf_rmse, 4),
            "notes": "Builds many decision trees and combines them. Useful for non-linear patterns and feature importance.",
            "feature_importance": feature_importance,
        }
    )
    ...
    return {
        "available": True,
        "message": (
            f"Ran first-pass regression models to predict '{target_column}'. "
            "These are exploratory and should not be treated as production models."
        ),
        "target": target_column,
        "task_type": "regression",
...
def detect_target_column(df, schema):
    ...
    # Then look for low-cardinality categorical columns.
...
def build_target_distribution(df, target_column):
    ...
    return [
        {
            "target_value": "Missing" if pd.isna(index) else str(index),
            "count": int(value),
            "percent": safe_round((value / max(len(df), 1)) * 100, 2),
        }
...
def build_target_by_category(df, schema, target_column):
    """
    For categorical columns, show how the target varies by group.

    For binary numeric targets like Titanic Survived:
    - calculate target mean by category, which acts like a rate.
    For categorical targets like Iris species:
    - calculate the most common target per category.
    """
    results = []
            ...
            results.append(
                {
                    "column": column,
                    "target": target_column,
                    "analysis_type": "target_rate_by_category",
                    "rows": [
                        {
                            "category": str(index),
                            "target_average": safe_round(row["mean"]),
                            "count": int(row["count"]),
                        }
            ...
            results.append(
                {
                    "column": column,
                    "target": target_column,
                    "analysis_type": "most_common_target_by_category",
                    "rows": [
                        {
                            "category": str(index),
                            "most_common_target": str(value),
                        }
...
def build_numeric_by_target(df, schema, target_column):
    """
    Compare numeric column averages across target groups.
    """
    results = []

    numeric_cols = [
        col for col in schema.get("numeric", [])
        if col != target_column
    ]
    ...
    for numeric_column in numeric_cols[:10]:
        ...
        grouped = (
            temp.groupby(target_column)[numeric_column]
                ...
                "rows": [
                    {
                        ...
                        "mean": safe_round(row["mean"]),
                        "median": safe_round(row["median"]),
                        "count": int(row["count"]),
                    }
                    for index, row in grouped.iterrows()
                ],
...
def boolean_to_numeric(series):
    """
    Convert real booleans or boolean-like strings into 0/1 values.
    """
    if pd.api.types.is_bool_dtype(series):
